...
def main():
    ...
    send_example_telemetry("run_prefixlm_parler_tts", model_args, data_args)
    ...
    config = PrefixLMParlerTTSConfig.from_pretrained(
    ...
    if training_args.codebook_weights is not None and len(training_args.codebook_weights) != config.decoder_config.num_codebooks:
        raise ValueError(f"`codebook_weights` has length {len(training_args.codebook_weights)} when it should be of length {config.decoder_config.num_codebooks}.")

    # update pad token id and decoder_start_token_id
    config.decoder_config.update(
        {
            "codebook_weights": training_args.codebook_weights if training_args.codebook_weights is not None else config.decoder_config.codebook_weights,
    ...
    # create model
    model = PrefixLMParlerTTSForConditionalGeneration.from_pretrained(
        model_args.model_name_or_path,
        cache_dir=model_args.cache_dir,
        config=config,
        token=data_args.token,
    ...
    audio_encoder_pad_token_id = config.decoder_config.pad_token_id
    audio_encoder_eos_token_id = config.decoder_config.eos_token_id
    audio_encoder_bos_token_id = model.generation_config.decoder_start_token_id
    max_length = model.generation_config.max_length
    num_codebooks = model.decoder.config.num_codebooks
    bandwidth = model_args.bandwidth
    attn_implementation = model_args.attn_implementation
    delay_strategy = config.decoder_config.delay_strategy
    ...
    def train_step(
        ...
        if mixed_precision == "fp16":
            ...
            with accelerator.autocast(autocast_handler=autocast_kwargs):
                ...
                if (
                    config.text_encoder.hidden_size != config.decoder_config.hidden_size
                    and config.decoder_config.cross_attention_hidden_size is None
                ):
    ...
    for epoch in range(epochs_trained, num_epochs):
        ...
        for _ in range(total_updates):
            ...
            if training_args.do_eval and (cur_step % eval_steps == 0 or cur_step == total_train_steps):
                ...
                eval_metrics = {
                    key: torch.mean(torch.cat([d[key] for d in eval_metrics])).to("cpu") for key in eval_metrics[0] # TODO: if loss isa 0-D tensor cannot work (if it's a number)
                }
